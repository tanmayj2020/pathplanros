...
class MaRRTPathPlanNode:
    ...
    def __init__(self):
        ...
        # Create publishers
        self.waypointsPub = rospy.Publisher(
            "/waypoints", WaypointsArray, queue_size=0)
    ...
    def mergeWaypoints(self, newWaypoints):
        ...
        if len(self.savedWaypoints) > 15:
            ...
            for waypoint in reversed(newWaypoints):
                distDiff = self.dist(
                    firstSavedWaypoint[0], firstSavedWaypoint[1], waypoint[0], waypoint[1])
            ...
            if i >= maxWaypointAmountToSave or carWaypointDist > maxDistToSaveWaypoints:
                ...
                for savedWaypoint in reversed(self.savedWaypoints):
                    waypointsDistDiff = self.dist(
                        savedWaypoint[0], savedWaypoint[1], waypointCandidate[0], waypointCandidate[1])
                    if waypointsDistDiff < waypointsDistTollerance:
                        self.savedWaypoints.remove(
                            savedWaypoint)  # remove similar
                        # print "remove this point:", savedWaypoint, "with diff:", waypointsDistDiff
                        break

                if (self.preliminaryLoopClosure):
                    distDiff = self.dist(
                        firstSavedWaypoint[0], firstSavedWaypoint[1], waypointCandidate[0], waypointCandidate[1])
                    if distDiff < waypointsDistTollerance:
                        self.loopClosure = True
        ...
        for i in range(len(filteredBranch) - 1):
            ...
            for edge in delaunayEdges:
                ...
                if self.getLineSegmentIntersection(a1, a2, b1, b2):
                    if edge.length() < maxAcceptedEdgeLength:
                        edge.intersection = self.getLineIntersection(
                            a1, a2, b1, b2)
            ...
            if intersectedEdges:
                ...
                if len(intersectedEdges) == 1:
                    ...
                    intersectedEdges.sort(key=lambda edge: self.dist(
                        node1.x, node1.y, edge.intersection[0], edge.intersection[1], shouldSqrt=False))
    ...
    def getDelaunayEdges(self, frontCones):
        ...
        for simp in tri.simplices:
            ...
            for i in range(3):
                ...
                if j == 3:
                    j = 0
                edge = Edge(conePoints[simp[i]][0], conePoints[simp[i]]
                            [1], conePoints[simp[j]][0], conePoints[simp[j]][1])
